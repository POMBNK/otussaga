...
func NewConsumer(rabbitMQ *mq.RabbitMQ, orderService *orderService.Service) *Consumer {
	return &Consumer{
		rabbitMQ:     rabbitMQ,
...
func (c *Consumer) ProcessFailedOrders(ctx context.Context) {
	...
	for {
		...
		case msg := <-msgs:
			...
			if err := c.orderService.RollbackOrder(ctx, orderID); err != nil {
